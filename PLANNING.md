# Architectural Plan & Specification: Omarchy Windows VM Widget (`io.github.yanuarpmbd.winvm`)
...
**Goal:** Create a first-class Omarchy Quickshell plugin (`io.github.yanuarpmbd.winvm`) that:
...
* **`install.sh`**:
  ...
  2. Creates directory `~/.config/omarchy/plugins/io.github.yanuarpmbd.winvm/`.
  3. Copies manifest, QML files, and assets.
  4. Triggers `omarchy-shell shell rescanPlugins`.
  5. Enables plugin via `omarchy plugin enable io.github.yanuarpmbd.winvm --section right`.
  6. Restarts shell via `omarchy restart shell`.
* **`uninstall.sh`**:
  1. Disables plugin with `omarchy plugin remove io.github.yanuarpmbd.winvm --yes`.
  2. Deletes `~/.config/omarchy/plugins/io.github.yanuarpmbd.winvm`.
